...
server <- function(input, output) {
  ...
  trade_table <- reactive({
    ...
    is.na(excess_pick1) <- do.call(cbind,lapply(excess_pick1, is.infinite))
  ...
  output$download <- downloadHandler(
    filename = function(){
      paste0(input$trade_up_team_id,
             ...
             " for Pick ",
             str_squish(str_to_title(unlist(strsplit(input$trade_up_picks_id,","))))[1],
             ".png")
    },
